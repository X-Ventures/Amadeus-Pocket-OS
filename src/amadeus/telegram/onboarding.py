...
def render_generic_capture_prompt(bot_ref: str) -> Text:
    return Text.assemble(
        f"  send /start to {bot_ref} in the chat you want amadeus to use "
...
async def step_default_engine(ui: UI, svc: Services, state: OnboardingState) -> None:
    ui.print("amadeus runs these engines on your computer. switch anytime with /agent.")
...
async def step_save_config(ui: UI, svc: Services, state: OnboardingState) -> None:
    ...
    ui.print("")
    ui.print(Text("✓ setup complete. starting amadeus...", style="green"))

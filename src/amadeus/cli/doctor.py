...
from ..settings import AmadeusSettings, TelegramTopicsSettings
...
def _doctor_file_checks(settings: AmadeusSettings) -> list[DoctorCheck]:
...
def _doctor_voice_checks(settings: AmadeusSettings) -> list[DoctorCheck]:
...
def run_doctor(
    *,
    load_settings_fn: Callable[[], tuple[AmadeusSettings, Path]],
    telegram_checks: Callable[
        [str, int, TelegramTopicsSettings, tuple[int, ...]],
        Awaitable[list[DoctorCheck]],
    ],
    file_checks: Callable[[AmadeusSettings], list[DoctorCheck]],
    voice_checks: Callable[[AmadeusSettings], list[DoctorCheck]],
) -> None:
    try:
    ...
    if settings.transport != "telegram":
        typer.echo(
            "error: amadeus doctor currently supports the telegram transport only.",
            err=True,
        )
...
def _resolve_cli_attr(name: str) -> object | None:
    cli_module = sys.modules.get("amadeus.cli")

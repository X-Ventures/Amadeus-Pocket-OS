...
from ..settings import AmadeusSettings, validate_settings_data
...
def _fail_missing_config(path: Path) -> None:
    ...
    if path.exists():
        typer.echo(f"error: invalid amadeus config at {display}", err=True)
    else:
        typer.echo(f"error: missing amadeus config at {display}", err=True)
...
def _normalized_value_from_settings(
    settings: AmadeusSettings, segments: list[str]
) -> Any:
    node: Any = settings
    for segment in segments:

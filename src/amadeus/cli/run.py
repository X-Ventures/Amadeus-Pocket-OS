...
from ..settings import AmadeusSettings, load_settings, load_settings_if_exists
...
def _load_settings_optional() -> tuple[AmadeusSettings | None, Path | None]:
    try:
...
def _should_run_interactive() -> bool:
    if os.environ.get("AMADEUS_NO_INTERACTIVE"):
        return False
...
def _run_auto_router(
    *,
    ...
    load_settings_fn = cast(
        Callable[[], tuple[AmadeusSettings, Path]],
    ...
    if debug:
        os.environ.setdefault("AMADEUS_LOG_FILE", "debug.log")

...
_suppress_below: ContextVar[int | None] = ContextVar(
    "amadeus_suppress_below", default=None
)
...
def setup_logging(
    *, debug: bool = False, cache_logger_on_first_use: bool = False
    ...
    level_name = os.environ.get("AMADEUS_LOG_LEVEL")
    if debug:
        level_name = "debug"
    _MIN_LEVEL = _level_value(level_name, default="info")

    trace_pipeline = _truthy(os.environ.get("AMADEUS_TRACE_PIPELINE"))
    _PIPELINE_LEVEL_NAME = "info" if trace_pipeline else "debug"

    format_value = os.environ.get("AMADEUS_LOG_FORMAT", "console").strip().lower()
    color_override = os.environ.get("AMADEUS_LOG_COLOR")
    ...
    log_file = os.environ.get("AMADEUS_LOG_FILE")

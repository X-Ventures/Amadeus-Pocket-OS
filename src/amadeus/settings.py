...
class TelegramTransportSettings(BaseModel):
    ...
    voice_transcription_model: NonEmptyStr = "whisper-1"
...
def load_settings(path: str | Path | None = None) -> tuple[AmadeusSettings, Path]:
...
) -> tuple[AmadeusSettings, Path] | None:
    cfg_path = _resolve_config_path(path)
    if cfg_path.exists():
...
def validate_settings_data(
    ...
    try:
        return AmadeusSettings.model_validate(data)
    except ValidationError as exc:
        raise ConfigError(f"Invalid config in {config_path}: {exc}") from exc


def require_telegram(settings: AmadeusSettings, config_path: Path) -> tuple[str, int]:
...
def _load_settings_from_path(cfg_path: Path) -> AmadeusSettings:
    cfg = dict(AmadeusSettings.model_config)
    cfg["toml_file"] = cfg_path
    Bound = type(
        "AmadeusSettingsBound",
        (AmadeusSettings,),
